...
data2, samplerate2=sf.read('./distorta.wav')
...
plt.title('Waveform 5: Distorta', size=30)
...
fig.suptitle('Fourier Analysis: Distorta', fontsize=35, y=1.05)
ax[0].set_title('Parte reale', fontsize=25)
ax[0].plot(freq2[:len(freq2)//2], r2[:len(r2)//2], color='royalblue')
ax[0].grid(linestyle=':')
...
ax[1].plot(freq2[:len(freq2)//2], i2[:len(i2)//2], color='royalblue')
ax[1].grid(linestyle=':')
...
ax[2].plot(freq2[:len(freq2)//2], p2[:len(p2)//2], color='royalblue')
ax[2].grid(linestyle=':')
...
plt.figure()
plt.title('Distorta: Spettro di potenza normalizzato', fontsize=25)
plt.plot(freq2[:len(freq2)//2], p2_norm[:len(p2)//2], color='royalblue', marker='*')
plt.grid(linestyle=':')
#plt.xlim(-50, 2500)
...
fig, ax=plt.subplots(1, 3, figsize=(15,6))
fig.suptitle('Larghezza di picchi principali: Distorta', fontsize=35, y=1.05)
ax[0].set_title('Primo picco', fontsize=25)
ax[0].plot(freq2[:len(freq2)//2], p2_norm[:len(r2)//2], color='royalblue', marker='*')
ax[0].grid(linestyle=':')
ax[0].set_xlabel('Frequenza [Hz]', fontsize=20)
ax[0].set_ylabel('|C_k|^2 norm', fontsize=20)
ax[0].set_xlim(-0.5, 2)
ax[1].set_title('Secondo picco', fontsize=25)
ax[1].plot(freq2[:len(freq2)//2], p2_norm[:len(i2)//2], color='royalblue', marker='*')
ax[1].grid(linestyle=':')
ax[1].set_xlabel('Frequenza [Hz]', fontsize=20)
...
ax[2].plot(freq2[:len(freq2)//2], p2_norm[:len(p2)//2], color='royalblue', marker='*')
ax[2].grid(linestyle=':')
ax[2].set_xlabel('Frequenza [Hz]', fontsize=20)
...
''' PARTE MANCANTE: FILTRAGGIO DEI PICCHI (QUALI?) E SINTENTIZZAZIONE DEL SEGNALE FILTRATO '''
